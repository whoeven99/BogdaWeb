...
- `ciwi-web/content/blog/*` 的当前 MDX 内容源

一期策略：

- 官网前台使用 `/blog` 作为统一入口
- 博客详情页统一由 `ciwi-web` 前台承接

当前实现进度：

- `/blog` 已作为统一入口页落地
- `/blog/[slug]` 已建立站内文章页模板
- 当前 Blog 内容已切换到仓库内 MDX 内容源
- 站内不再保留 Ghost 原始链接入口
...
- 高价值文档详情页统一由 Next 前台承接
- 历史静态帮助中心站点已清理，仅保留当前页面仍在使用的素材图
