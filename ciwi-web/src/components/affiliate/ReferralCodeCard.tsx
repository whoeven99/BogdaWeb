      ...
      {showUsage ? (
        <div className="modal-overlay" onClick={() => setShowUsage(false)}>
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="referral-usage-title"
            onClick={(event) => event.stopPropagation()}
          >
            <h3 id="referral-usage-title">{copy.usageTitle}</h3>
            <p>{copy.usageDescription}</p>
            <button type="button" className="button button--primary" onClick={() => setShowUsage(false)}>
              {copy.usageConfirmLabel}
            </button>
          </div>
        </div>
      ) : null}
