...
本节为最新视觉基线，与后文旧尺寸或颜色说明冲突时以本节为准。

- 参考 DesignMD 的公开方案介绍：[Genesis](https://designmd.ai/chef/genesis)（标题与卡片层次）、[Ember Studio](https://designmd.ai/chef/ember-studio)（暖中性色）。完整 DESIGN.md 因需授权未读取，不声称已按其完整规范实现。
- 保留 Ciwi 品牌绿，用偏白画布、薄荷与雾紫低饱和光感、暖杏展示面板构成视觉体系；不全站改为彩色卡片或玻璃面板。
- `src/styles/tokens.css` 管理颜色、渐变和阴影；`src/styles/surfaces.css` 管理共享表面、导航与卡片。禁止在新组件中再次复制大圆角与长阴影字符串。
- `--gradient-page` 用于站点顶部画布；`--gradient-surface` 仅用于显式产品展示变体；`--gradient-mint / pearl / warm` 用于营销展示与章节背景；`--gradient-dark` 仅用于有意强调的深色模块。
- 默认卡片 12px 圆角、展示面板 16px、按钮 8px。头像、状态点和分类标签仍可使用圆形或胶囊形。
- `Button` 默认最小高度 44px / 14px 字号，`size="large"` 最小高度 52px / 15px 字号，统一 600 字重、零字距，长文案允许换行。主按钮使用低饱和灰绿色 `--color-action-primary` 与白字；hover、按下仅逐级降低明度。按钮禁止渐变、高光、外阴影及按压内阴影，保持扁平。不使用 `!important` 覆盖交互状态。
- 次按钮及卡片入口使用白底、中性细边框，悬停为浅灰绿；深底反色按钮使用近白纯色。主次靠填充与明度区分，键盘焦点继续使用清晰轮廓。章节与演示背景仍可使用渐变，不将背景光感扩散到操作控件。
- `ui-editorial-card` 为文章、场景、评价和产品卡片共享外壳；产品卡可使用 `ui-editorial-card--product` 弱光感变体。
- 内容型组件默认平面：资源卡、目录、摘要、FAQ、步骤、帮助中心面板采用白底或浅灰底、细边框，无渐变和悬浮阴影；hover 仅改变边框。`ui-panel--muted` 表示浅灰面板，不使用配色命名的渐变变体。
- 文章提示框保留警告色语义，但用纯色底。关键词流程不放无信息的渐变占位面板。内容页普通页尾 CTA 使用浅灰底，首页及产品页的营销展示与 CTA 可保留专用渐变。
- `ContentIndexHero` 内部复用 `SectionHeading`。普通区块标题为 28–38px，Hero 为 36–60px；正文 16px，导语 17–20px，辅助信息 14px。产品页可保留更强的营销标题层级。
- 共享卡片标题 20–22px / 600 / 1.35 行高；区块标题与 Hero 700 字重；正文 400 / 1.75 行高；分类标签 12px。字号与字重优先引用 `--text-*` 和 `--weight-*`，不以整页字体放大代替层级设计。继续使用系统无衬线字体栈，不新增远程字体下载。
- 正文和长文章不加渐变文字、不反复穿插深色背景。帮助中心保留检索和目录逻辑；后台表格和业务状态颜色不由装饰配色替代。
- 产品页整宽渐变只在背景伪元素上绘制一次，避免版心内外渐变接缝；深底正文必须使用浅色高对比文字。
- 保留可见键盘焦点和减少动态效果偏好。导航仅使用静态半透明模糊，不新增持续运行的光效动画。
- 验收覆盖首页、产品列表与详情、资源列表、帮助中心、关键词详情的中英文和窄屏；不能以构建通过替代浏览器检查。
