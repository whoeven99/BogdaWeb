...
class PrometheusClient {
     ...
     */
    constructor(prometheusConfig = {}, logger = logging_1.default) {
        ...
        if (this.config.pushgateway?.enabled && this.gateway) {
            ...
            try {
                ...
                if (typeof err === 'object' && err !== null && 'code' in err) {
                    error = 'Error pushing metrics to pushgateway: ' + err.code;
                }
                else {
    ...
    instrumentKnex(knexInstance) {
        ...
        knexInstance.client.pool.on('createRequest', (eventId) => {
            this.creates.set(eventId, createDurationSummary.startTimer());
        });
        knexInstance.client.pool.on('createSuccess', (eventId) => {
            this.creates.get(eventId)?.();
            this.creates.delete(eventId);
        });
        knexInstance.client.pool.on('createFail', (eventId) => {
            this.creates.get(eventId)?.();
            this.creates.delete(eventId);
        });
        knexInstance.client.pool.on('acquireRequest', (eventId) => {
            this.acquires.set(eventId, acquireDurationSummary.startTimer());
        });
        knexInstance.client.pool.on('acquireSuccess', (eventId) => {
            this.acquires.get(eventId)?.();
            this.acquires.delete(eventId);
        });
        knexInstance.client.pool.on('acquireFail', (eventId) => {
            this.acquires.get(eventId)?.();
            this.acquires.delete(eventId);
        });
    }
