...
function emailCTATemplate(dataset, options = {}) {
  ...
  let buttonStyle = dataset.buttonColor === 'accent' ? `color: ${dataset.buttonTextColor};` : `background-color: ${dataset.buttonColor}; color: ${dataset.buttonTextColor};`;
  ...
  if ((options?.feature?.emailCustomization || options?.feature?.emailCustomizationAlpha) && options?.design?.buttonStyle === 'outline' &&
...
function renderCallToActionNode(node, options = {}) {
  ...
  if (dataset.hasSponsorLabel) {
    const cleanBasicHtml = buildCleanBasicHtmlForElement(element);
    const cleanedHtml = cleanBasicHtml(dataset.sponsorLabel, {
      firstChildInnerContent: true
    });
          ...
          conversion(domNode) {
            ...
            if (imageElement) {
              const {
                ...
                width,
                height
              } = readImageAttributesFromElement(imageElement);
...
var taggedTemplateFns = /*#__PURE__*/Object.freeze({
    __proto__: null,
    html: html,
    oneline: oneline
});
...
function renderButtonNode(node, options = {}) {
  ...
  if (options.target === 'email') {
    return emailTemplate$3(node, options, document);
  } else {
...
function emailTemplate$3(node, options, document) {
  const {
    buttonUrl,
    buttonText
  } = node;
...
function cardTemplate$2(nodeData, options = {}) {
...
function renderHeaderNodeV2(dataset, options = {}) {
  ...
  const htmlString = cardTemplate$2(node, options);
  const element = document.createElement('div');
  element.innerHTML = htmlString?.trim();
  if (node.header === '') {
